...
class TableNode(Node):
    def __init__(self, name: str, scope: Scope | exp.Expression, schema = None):
        super().__init__(name)
...
class LineageMap:
    ...
    def _parse_scope(self, scope: Scope | exp.Table, name = None) -> TableNode:
        ...
        if isinstance(scope.expression, exp.Select):
            ...
            for name, source in scope.sources.items():
                childtable = self._parse_scope(source, name)

                root.sources[name] = childtable
                root.downstream.append(childtable)
                childtable.upstream.append(root)
    ...
    def _parse_create_table(self, create: exp.Create):
        ...
        root = TableNode(table.name, scope = create, schema=table.db)
